<header class="sticky top-0 z-50 bg-white/80 backdrop-blur border-b border-gray-200">
    <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        {{-- Logo --}}
        <a href="{{ localizeRoute('welcome') }}" class="flex items-center gap-3">
            <img src="{{ asset('icon.png') }}" alt="Forksy" class="h-9 w-auto">
        </a>

        {{-- Right side --}}
        @guest
            <div class="flex items-center gap-3">
                <a href="{{ localizeRoute('auth.login.show') }}"
                   class="text-sm font-medium text-gray-600 hover:text-gray-900 transition">
                    {{ __('header.login') }}
                </a>

                <a href="{{ localizeRoute('auth.register.show') }}"
                   class="inline-flex items-center rounded-full bg-green-600 px-5 py-2 text-sm font-medium text-white
                          hover:bg-green-700 transition shadow-sm cursor-pointer">
                    {{ __('header.register') }}
                </a>
            </div>
        @endguest

        @auth
            <div class="relative" x-data="{ open: false }">
                {{-- Avatar button --}}
                <button
                    @click="open = !open"
                    @click.outside="open = false"
                    class="flex items-center gap-2 rounded-full cursor-pointer focus:outline-none"
                >
                    <img
                        src="{{ auth()->user()->getAvatarPath() }}"
                        alt="Avatar"
                        class="h-9 w-9 rounded-full object-cover border border-gray-200"
                    >
                </button>

                {{-- Dropdown --}}
                <div
                    x-show="open"
                    x-transition
                    class="absolute right-0 mt-2 w-48 rounded-xl bg-white shadow-lg border border-gray-200 overflow-hidden"
                >
                    <div class="px-4 py-3 border-b border-gray-100">
                        <p class="text-sm font-medium text-gray-900 truncate">
                            {{ auth()->user()->name }}
                        </p>
                        <p class="text-xs text-gray-500 truncate">
                            {{ '@'.auth()->user()->username }}
                        </p>
                    </div>

                    <div class="py-1">
                        <a href="{{ localizeRoute('welcome') }}"
                           class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition">
                            {{ __('header.profile') }}
                        </a>

                        <a href="{{ localizeRoute('user.settings') }}"
                           class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 transition">
                            {{ __('header.settings') }}
                        </a>
                    </div>

                    <div class="border-t border-gray-100">
                        <form method="POST" action="{{ localizeRoute('auth.logout') }}">
                            @csrf
                            <button
                                type="submit"
                                class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-gray-100 transition cursor-pointer"
                            >
                                {{ __('header.logout') }}
                            </button>
                        </form>
                    </div>
                </div>
            </div>
        @endauth
    </div>
</header>
